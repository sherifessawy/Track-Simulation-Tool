def simulator(vi, track): #vi is initial velocity at the beginning of the simulation, track is a dictionary of form {1:['s', lenght], 2:['c', length, raduis]}

    """
    simulates the track calling the three main functions(straight line, corner, brake) appropriately
    
    """
    i = 0     #ends the simulation when i == len(track)
    ...
    xx[i] = a    #add solution to xx dict
                 #a contains t, v, list_t, list_v, list_s of the function called (the solution of the road is added to dic xx)
    i += 1
    ...
    while i < len(track): #will end itself when track end reached
        if track[i][0] == 'c':                                 #'c' for corner, so call corner function
            a = corner(xx[i-1][1], track[i][1], track[i][2])
            if len(a) == 1:                                    
                """ it means the corner function returned only the max velociy of the corner 
                    as the car velocity at the entrance was larger than the corner max velocity 
                    so it will call the brake function as explained in the corner function to update the rejected solution """
                ...
                """ prevouis straight road or corner solution is accepted 
                (i.e, velocity at entrance is less than the corner max velocity)
                 so add directly the corer solution to xx dict """
                xx[i] = a
                i += 1
                

        else: #current part of track is a straight road
            a = straight_line(xx[i-1][1], track[i][1])  #call straigh road function; arguments taken (straight_line(vi, length))
            xx[i] = a                                   #add straight road solution to xx dict
    ...
    while i < len(xx):
        ...
        if xx[i][-1] == 's':
            lateral_acc += [0]*len(xx[i][3])            #lateral acc in straight roads = zero
        elif xx[i][-1] == 'c':
            for e in xx[i][3]:
                lateral_acc += [(e**2/(xx[i][5]*9.81))] #calculate lateral acceleration in corners
        ...
        if i == 0:
            ...
            s_simulation += s_simulation_temp                          #then concatenate the lists (the distances)
            v_simulation += xx[i][3]                                   #evaluate final velocity list during the whole simulation
            t_simulation_temp = [x+t_simulation[-1] for x in xx[i][2]] #to accumelate time
            t_simulation += t_simulation_temp                          #time taken during the whole simulation
            i += 1

    print('lap time', round(t_simulation[-1],3), 'sec')
    print('velocity at end', round(v_simulation[-1],2), 'm/s')   
    #return round(t_simulation[-1],3)
    return [s_simulation, v_simulation,round(t_simulation[-1],3), lateral_acc] #list of the whole track for distance and corresponding velocity and lateral acceleration, also returns lap time
